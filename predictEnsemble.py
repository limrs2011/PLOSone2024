...
def fusion(img1, img2, device):
	# Mean
	imgFusion = (torch.sigmoid(img1.to(device)) + torch.sigmoid(img2.to(device)))/2
	...
	filt2 = img1.to(device).clone().detach().type(torch.int8) & img2.to(device).clone().detach().type(torch.int8)
...
def weighted_PCA(img1, img2, device):
	# 判斷img tensor大小
	if len(img1.size()) == 4:
		img1 = img1.squeeze(0)
		img2 = img2.squeeze(0)
	...
	img1_centered = img1_2d - torch.mean(img1_2d)  # 中心化 img1
	img2_centered = img2_2d - torch.mean(img2_2d)  # 中心化 img2
	...
	if max_eigenvalue1 + max_eigenvalue2 == 0:
		omega1 = omega2 = 0.5
	else:
		omega1 = max_eigenvalue1 / (max_eigenvalue1 + max_eigenvalue2)
		omega2 = max_eigenvalue2 / (max_eigenvalue1 + max_eigenvalue2)
	...
	imgFusion = ( (torch.sigmoid(img1.to(device))*omega1) + (torch.sigmoid(img2.to(device))*omega2) ) / (omega1 + omega2)
...
def predict_img(net,
	...
	with torch.no_grad():
		...
		# new = weighted_PCA(output, output2, device)
		# new = fusion(output, output2, device)
		new = union(output, output2, device)
